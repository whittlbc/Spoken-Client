...
- The space character.
- Punctuation characters and other symbols, including: "!", "#", "$", "%", "&", "(", ")", "+", "-", ":", ";", "<", "=", ".", ">", "?", "@", "[", "]", "^", "_", "{", "}", "|", "~", ",".
@param token The token generated at you server. For details, see [Generate a token](https://docs.agora.io/en/Interactive%20Broadcast/token_server?platform=All%20Platforms).
 ...
 *Use Cloud Proxy* to add the specific IP addresses and ports to the firewall 
 whitelist; then, call this method to enable the cloud proxy and set the `proxyType` 
 parameter as `AgoraUdpProxy(1)`, which is the cloud proxy for the UDP protocol.
 ...
 * YES: Route the audio to the speakerphone. If the playback device connects to the headset or Bluetooth, the audio cannot be routed to the speakerphone.
 ...
 **Note:**
 ...
 - Call this method after calling [startAudioMixing]([AgoraRtcEngineKit startAudioMixing:loopback:replace:cycle:]) 
and receiving the [localAudioMixingStateDidChanged]([AgoraRtcEngineDelegate rtcEngine:localAudioMixingStateDidChanged:errorCode:])(`AgoraAudioMixingStatePlaying`) callback.
 ...
 **Note**
 ...
 @param delegate The delegate of the audio frame. See AgoraAudioFrameDelegate.
...
- (void)remoteAudioStatBlock: (void (^ _Nullable)
												...
												NSInteger qoeQuality,
												NSInteger qualityChangedReason,
												NSInteger mosValue)) remoteAudioStatBlock __deprecated_msg("use delegate instead.");
